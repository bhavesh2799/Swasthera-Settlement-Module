...
import { ArrowUpRight, Activity, Clock, CheckCircle2, AlertCircle, BanknoteIcon, Download, FileText, Loader2, ChevronLeft, ChevronRight } from "lucide-react";
...
export function Dashboard() {
  ...
  const settlementPageCount = Math.max(1, Math.ceil(totalSettlements / SETTLEMENT_PAGE_SIZE));
  const pagedSettlements = settlements?.slice(
    settlementPage * SETTLEMENT_PAGE_SIZE,
    settlementPage * SETTLEMENT_PAGE_SIZE + SETTLEMENT_PAGE_SIZE,
  );
      ...
      <div className="grid gap-6 md:grid-cols-7">
        <Card className="md:col-span-5 shadow-sm border-slate-200/60 bg-white">
          ...
          <CardContent className="p-0 flex flex-col h-[360px]">
            ...
            {totalSettlements > SETTLEMENT_PAGE_SIZE && (
              <div className="flex items-center justify-between border-t border-slate-100 px-4 py-2.5 bg-slate-50/50">
                <span className="text-xs text-slate-500">
                  {settlementPage * SETTLEMENT_PAGE_SIZE + 1}–{Math.min((settlementPage + 1) * SETTLEMENT_PAGE_SIZE, totalSettlements)} of {totalSettlements}
                </span>
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-slate-500 px-1">{settlementPage + 1}/{settlementPageCount}</span>
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-7 px-2"
                    disabled={settlementPage >= settlementPageCount - 1}
                    onClick={() => setSettlementPage((p) => Math.min(settlementPageCount - 1, p + 1))}
                  >
        ...
        <Card className="md:col-span-2 shadow-sm border-slate-200/60 bg-white">
          ...
          <CardContent className="p-4 h-[360px] overflow-y-auto">
